...
`pnpm dev` で起動したセットアップ画面には「ローカル検証ツール」が表示されます。投手戦、乱打戦、サヨナラ、延長戦、DHでの投手交代、代走・盗塁・代打など、進行済みの試合パターンを選んで各画面と集計を確認できます。このツールとサンプル選手のプリセットは開発環境専用で、`pnpm build` した本番環境には表示されません。
